 ...
 */
export const DeleteHistoryFile = ({ swaggerDocs }: EndpointOptions = { swaggerDocs: true }) => {
...
function getSwaggerDefinitions(swagger: Swagger) {
  ...
  const NotFound = ApiNotFoundResponse({
    description: 'Execution history content not found',
    schema: getErrorSchema({ status: 404, message: 'no execution history file found for version_id <id>' }),
  });

  const Ok = ApiNoContentResponse({ description: 'Execution history file deleted successfully' });
